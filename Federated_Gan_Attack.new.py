...
def main():
    ...
    def train_step(data_sample_batch, data_labels):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            ...
            if r == 0:
                 gradients_of_generator0 = gen_tape.gradient0(gen_loss, generator.trainable_variables)### difrentiated difinition
                 gradients_of_discriminator0 = disc_tape.gradient0(disc_loss, malicious_discriminator.trainable_variables)### difrentiated difinition
    
                 generator_optimizer.apply_gradients0(zip(gradients_of_generator0, generator.trainable_variables))
                 discriminator_optimizer.apply_gradients0(zip(gradients_of_discriminator0, malicious_discriminator.trainable_variables))

            else:
                gradients_of_generator1 = gen_tape.gradient1(gen_loss, generator.trainable_variables)  ### difrentiated difinition
                gradients_of_discriminator1 = disc_tape.gradient1(disc_loss, malicious_discriminator.trainable_variables)  ### difrentiated difinition

                generator_optimizer.apply_gradients1(zip(gradients_of_generator1, generator.trainable_variables))
                discriminator_optimizer.apply_gradients1(zip(gradients_of_discriminator1, malicious_discriminator.trainable_variables))
